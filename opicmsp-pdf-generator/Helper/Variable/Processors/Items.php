...
class Items extends AbstractHelper
{
     ...
     * @return string
     */
    public function variableItemProcessor($standardItem, $template, $bar_code = false)
    {
        ...
        if($orderItem->getPrice() == 0 || $item->getPrice() == 0){
            $orderItem->setPrice(' ');
            $item->setPrice(' ');
        }
        $transport = [
    ...
    public function processItems($source, $templateModel)
    {
        ...
        foreach ($items as $item) {
            //DINO-5 fix to template to show all child products to inviuse print
...
//            }
            ...
            $processedItem = $this->variableItemProcessor($item, $itemBodyParts, $bar_code);
